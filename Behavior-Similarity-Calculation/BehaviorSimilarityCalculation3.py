# python BehaviorSimilarityCalculation3.py --labels_path ./train_labels
import os
import argparse
...
def find_matching_rows(arr):
    ...
    for i in range(rows):
        ...
        for j in range(i+1, rows):
            ...
            if compute_iou(current_bbox, next_bbox) > 0.9:
                # 匹配成功，记录数量
     ...
     for name in files:
        if "checkpoint" not in name and ".txt" in name :
            ...
            for key in count_overlap:
                ...
                if len(count_overlap) > 0 :
                    if key == '1 2':
                        specific_overlap_files.append(name.split('.')[0])
                        # print(name.split('.')[0])
                        # temp_num = temp_num + 1
...
for overlap in all_count_overlap:
    print(f"{overlap}:   ",end='')
    for  category in  all_count_category:
        if str(category) in overlap.split(" "):
#             print(category," ", format(all_count_overlap[overlap]/all_count_category[category],'.3f')," ",end='')
            print(format(all_count_overlap[overlap]/all_count_category[category],'.3f')," ",end='')
    print()
            

print("specific_overlap_files:",specific_overlap_files)